...
const PaginationComponent = ({
  ...
  const handlePageClick = (selectedPageNum) => {
    const newOffset =
      ((selectedPageNum - 1) * boardgamesPerPage) % boardgames.length
    console.log(
      `User requested page number ${selectedPageNum}, which is offset ${newOffset}`
    )
  ...
  return (
    <>
      <StyledContainer>
        <Pagination>
          <Pagination.First onClick={handleButtonClick} disabled={page === 1} />
          <Pagination.Prev onClick={handleButtonClick} disabled={page === 1} />
